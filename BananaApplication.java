...
public class BananaApplication extends JFrame implements KeyListener, ActionListener, MouseListener{
	...
	public enum State {openning, starting, selecting, weapons, playing, ending, won, lost, shop}
	State gameState;
	boolean move, gridTime;


	int r,b,gr, lives,l, t, coins, levelRow, levelNumber, levelTime, maxLevel, totalPotions;
...
//	AudioClip nyan, laser, coinSound, pop;
	...
	ArrayList<String> weaponStrings;
...
/* try {
		...
		shop = new Shop();
		try{
			back = new RoundButton(new ImageIcon(ImageIO.read(getClass().getResource("/return.png"))));
		}catch(IOException e){}
		add(back);
...
//		laser = getAudioClip(getDocumentBase(), "Laser_Blast.wav");
	...
	//	pop = getAudioClip(getDocumentBase(), "pop.wav");
		...
		weaponStrings = new ArrayList<String>();
		try{
					yImage=ImageIO.read(getClass().getResource("/y.png"));
		 }catch(IOException e){}


		try {
			banana = ImageIO.read(getClass().getResource("/goodBanana.png"));
	...
	*/		for(int x = 1; x<5; x++){
				portalImages.add(ImageIO.read(getClass().getResource("p"+x+".png")));				// THESE SHOULD BE GONE
				redPortalImages.add(ImageIO.read(getClass().getResource("rp"+x+".png")));
				greenPortalImages.add(ImageIO.read(getClass().getResource("gp"+x+".png")));
				bluePortalImages.add(ImageIO.read(getClass().getResource("bp"+x+".png")));
				tealPortalImages.add(ImageIO.read(getClass().getResource("t"+x+".png")));
				yellowPortalImages.add(ImageIO.read(getClass().getResource("y"+x+".png")));
			}
			for(int x = 4; x>0; x--){
				portalImages.add(ImageIO.read(getClass().getResource("p"+x+".png")));
				redPortalImages.add(ImageIO.read(getClass().getResource("rp"+x+".png")));
				greenPortalImages.add(ImageIO.read(getClass().getResource("gp"+x+".png")));
				bluePortalImages.add(ImageIO.read(getClass().getResource("bp"+x+".png")));
				tealPortalImages.add(ImageIO.read(getClass().getResource("t"+x+".png")));
		...
		try{
			...
			while(scan.hasNext()){
				coins=Integer.parseInt(scan.next());
			//	//(coins);
			}

			FileReader weaponsReader = new FileReader("weapons.jake");
			scan = new Scanner(weaponsReader);
			while(scan.hasNextLine()){
				weaponStrings.add(scan.nextLine());
			}
			for(String s:weaponStrings){
			//	//(s);
			}
			InputStream istream = getClass().getClassLoader().getResourceAsStream("level.jake");
			BufferedReader levelReader = new BufferedReader(new InputStreamReader(istream));
			Scanner scan2 = new Scanner(levelReader);
			while(scan2.hasNext()){
								maxLevel=Integer.parseInt(scan2.next());
				}
	...
	//		System.err.println("nope");
		...
		for(ArrayList<Item> buttons: items){
			for(Item item: buttons){
				for(String s:weaponStrings){
					if(s.equalsIgnoreCase(item.getTitle()))item.unlock(false);
				}
			}
		}
		...
		fruit = new myCharacter(banana, square, 15, this);
		...
		coinCount = new CoinCounter(480,10,coins);
		potCount = new potionCounter(350,10);
		...
		setSize(square);
	//	boolean[] bools = {false,true,true,true,true,true,true,true,true,true,true,true,true,true,true,true,true};

		goTime = new JButton("GO TIME");
		goTime.addActionListener(this);

		gameState=State.openning;
		//gameState=State.starting;


		ArrayList<Item> iterms = new ArrayList<Item>();
								for(ArrayList<Item> buttons:items){
									for(Item i:buttons){
										iterms.add(i);
									}
								}
		ws = new WeaponSelect(iterms);
...
//==================================   DRAW SOMETHING   ======================
	...
	public class drawingPanel extends JPanel{
		...
		//	if(lives>0){
				if(gameState==State.openning){
					if(comic.isDone()) gameState=State.starting;
					comic.drawComic(g2);
				...
				//	//("Doing everything");
					...
					for(ArrayList<Item> buttons: items){
						for(Item item: buttons){
							...
							if(item.getModel().isSelected()){
								...
								//	buy.setFont( new Font(attributes) );
									Font font =new Font("SansSerif",Font.BOLD,16).deriveFont(map);
									g2.setFont(font);
								}
								else
								buy.setEnabled(true);

								if(item.getPrice()>coins && item.isLocked()){
									g2.setColor(Color.red);
									buy.setEnabled(false);
								}
								g2.drawString("Price: $"+item.getPrice(),730,550);
							}
						}
						bx+=95;
						by=165;
				...
				else if(gameState==State.weapons){
					...
					ws.setVisible(true);
					ws.validate();
					ws.setBounds(0,0,886,663);
					add(ws);
					setBackground(Color.white);

					goTime.setVisible(false);
					goTime.setBounds(775,10,90,35);
					goTime.setVisible(true);
...
//-------   -----------   ----------   ---------   --------   ------------   -----------
				...
				else if(gameState==State.playing||gameState==State.ending){		   //Start Game
...
//
	...
	//POTION GRID------------------------------------------------------------------------------------------------------
				...
				//	//("collected ="+potCount.getCollected());
				//	//("potionsLeft ="+pgrid.potionsLeft());
					if(potCount.isSet()&&potCount.noDen()){
						potCount.setPotionCount(pgrid.potionsLeft());
							totalPotions=pgrid.potionsLeft();
						}
					if(gridTime){
						pgrid.setChange(fruit.getXChange(), fruit.getYChange());
						pgrid.setBoundary(map);
						pgrid.drawGrid(g2);
						potCount.addCollected(pgrid.isCollision());
					//	pgrid.checkCollision();
						if(!potCount.noDen())potCount.setDen(pgrid.potionsLeft());
						pgrid.checkInside();
						if(!potCount.isSet()){
							potCount.ready();
					//		//("setting");
						...
						potCount.setNum(totalPotions-pgrid.potionsLeft());
						potCount.drawPotionCounter(g2);
				//		for(int i =0; i <currentLevel.getLaser().get(levelNumber-1).length; i++){
...
//-------- ------- ------- ------- ------ ------ --------- --------- -------- ------- ------ ------
			...
			//	gameState = State.starting;
				fruit.reset();
				try{
					g.drawImage(ImageIO.read(getClass().getResource("LoseScreen.png")),0,0,this);
				}catch(IOException e){}
				repaint();
...
//====================================   KEY FUNCTIONS ===============================
			...
			if(e.getKeyCode()==KeyEvent.VK_SPACE && !comic.getState().equalsIgnoreCase("done")){
				comic.next();
			}
...
//======================================CONTROL SOUND BUTTON====================
		public void mouseClicked(MouseEvent m){
			...
			if(m.getSource()==back){
				...
				back.setBounds(5,2,55,55);
				lev.setVisible(false);
				if(ws!=null)ws.setVisible(false);
			//	//(back.isVisible());
				repaint();
				if(ws!=null&&ws.isVisible())ws.setVisible(false);
				goTime.setVisible(false);
			}
	...
	//	}
		...
		ws = new WeaponSelect(iterms);
			...
			//	//("fdsaf");
				ws.setVisible(false);
				goTime.setVisible(false);
	//			for(int i =0; i<ws.getWeapons().length; i++){
	//				//(ws.getWeapons()[i].getDescription());
	//			}
				fruit.setLaser(ws.getWeapons());
				buy.setVisible(false);


			}
			if(e.getSource()==gameTimer && gameState==State.ending){
				if(lives==0 || pgrid.potionsLeft()>0)
					scale-=0.02;
				else{
				...
				repaint();
			//	//(scale);
				if(scale<=0||scale>=4){scale=1; endLevel();}
			}
...
//						coinSound.play();
	...
	/*						Area eba = new Area(l);
							...
							ba.intersect(eba);
								...
								if(!ba.isEmpty())System.out.println("intersecting");
								System.out.println(l.getHealth());
								if(!ba.isEmpty()&&l.getHealth().equalsIgnoreCase("good"))fruit.loseLife();

//							}*/
							for(Rectangle r: fruit.getCollisionBoxes()){
								if(l.intersects(r))fruit.loseLife();
							}
	...
	*/// COUNTDOWN AND LASERS
			...
			//	//("tick");
				for(Laser laz : currentLevel.getLaser().get(levelNumber-1)){
					...
					else if(t%laz.getDelay()-(laz.getDelay()/20)==0)laz.shutOff();
					else if(t%laz.getDelay()<=(laz.getDelay()/2)&&t%laz.getDelay()>=-(laz.getDelay()/2))laz.addCharge();
					if(t%laz.getDelay()+(laz.getDelay()/6)==0)laz.shutOff();
					laz.moveCharges();
					laz.drawCharges(g2);
			//		//(laz.getState());
					}/*
...
//=========================================   OTHER METHODS   ==========================================
	...
	public void setT(int[][] freqs){
		bliffT=freqs[levelNumber-1][1];
		shmockT=freqs[levelNumber-1][2];
		pangaT=freqs[levelNumber-1][3];
		coinT = /*(int)coinFrequency/gameTimer.getDelay();*/1;
		miniT = pangaT;
		glooshT = freqs[levelNumber-1][4];
		slimeT = glooshT;
		amyT = freqs[levelNumber-1][5];
		eyeraT = freqs[levelNumber-1][0];
	//	//("startingT");
	}
	...
	public void endLevel(){
		...
		if(lives>0 && pgrid.potionsLeft()<=0){
			...
			File f = new File("level.jake");
			try
			{
				FileWriter fstream = new FileWriter(f.getAbsoluteFile());
				BufferedWriter bout = new BufferedWriter(fstream);
						if(levelNumber>maxLevel)bout.write(""+levelNumber);

				bout.close();


							FileReader levelReader = new FileReader(new File("level.jake").getAbsoluteFile());
							Scanner scan2 = new Scanner(levelReader);
							while(scan2.hasNext()){
												maxLevel=Integer.parseInt(scan2.next());
								}
...
//		shoot3=false;
	...
	public void startLevel(int levNum){
			...
			levelNumber = levNum;currentLevel = new Level(levelNumber);
							...
							levelTime = currentLevel.getTimes()[levelNumber-1];
